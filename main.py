...
# ── Contact info (fetched dynamically, never hardcoded in frontend) ──
...
def wallet_deposit():
    ...
    try:
        if method == "card":
            ...
            return jsonify({"status": "success", "message": "STK push sent. Enter PIN."})
...
# ── AI Support Chat (single, clean, deep knowledge) ──
...
def support_chat():
    ...
    if "hello" in message or "hi" in message or "hey" in message:
        reply = "Hello! 👋 I'm the Jvex Labs virtual assistant. How can I help you today? You can ask me about our services, products, your account, payments, freelancing, and more."
    elif "deposit" in message:
        reply = "To deposit funds, go to your Dashboard > Overview. Use Card Deposit (Visa/Mastercard) or M‑Pesa Deposit. Funds appear instantly."
    elif "withdraw" in message:
        reply = "To withdraw to M‑Pesa, go to Dashboard > Overview > Withdraw. Enter your phone number and amount. Admin approves quickly."
    elif "balance" in message:
        reply = "Your balance is shown on the Dashboard Overview. You can also view all transactions in Inbox."
        ...
        reply = "Your unique referral link is in Dashboard > Teams. Share it with friends to earn commissions based on your tier."
        ...
        reply = "When buying a service, you can choose from 4 templates (Starter, Standard, Premium, Enterprise) with a 30% discount!"
    elif "sign" in message or "login" in message:
        reply = "You can sign up for free on our homepage. If you already have an account, go to /login. We also support Google sign‑in."
    elif "refund" in message:
        reply = "Refund policies are available in our Terms (footer). Generally, refunds are processed within 5‑10 business days."
